...
import { getAllProducts } from "../lib/d1-client";
...
export const generateBatchReport: RequestHandler = async (req, res) => {
  try {
    ...
    console.log(`[generateBatchReport] Using ${productData.length} products from D1`);
    ...
    for (const batch of batches as BatchItem[]) {
      ...
      for (const code of batch.codes) {
        ...
        if (product) {
          foundCount++;
          const unitPrice = product.price_final || 0;
          const unitPriceWithIPI = product.price_final_with_ipi || 0;
  ...
  try {
    ...
    for (const batch of batches as BatchItem[]) {
      ...
      for (const code of batch.codes) {
        ...
        if (product) {
          batchFoundCount++;
          const unitPrice = product.price_distributor || 0;
          const unitPriceWithIPI = product.price_distributor_with_ipi || 0;
          const totalPrice = product.price_final ? product.price_final * batch.quantity : 0;
          const totalPriceWithIPI = product.price_final_with_ipi ? product.price_final_with_ipi * batch.quantity : 0;
          const priceMultiplied = (product.price_final_with_ipi || 0) * finalMultiplier;
    ...
    for (const batch of batches as BatchItem[]) {
      ...
      for (const code of batch.codes) {
        ...
        if (product) {
          ...
          const unitPrice = product.price_distributor || 0;
          const unitPriceWithIPI = product.price_distributor_with_ipi || 0;
          const totalPrice = (product.price_final || 0) * batch.quantity;
          const totalPriceWithIPI = (product.price_final_with_ipi || 0) * batch.quantity;
          const priceMultiplied = (product.price_final_with_ipi || 0) * finalMultiplier;
